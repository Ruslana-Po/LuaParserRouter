...
std::string StringManipulation::ProcessString(const std::string& input)
{
    ...
    auto new_end =
        unique(result.begin(), result.end(), [](char a, char b) { return a == ' ' && b == ' '; });
...
bool StringManipulation::CheckWithThenString(std::string& str, const std::string& conOperator)
{
    ...
    {
        throw ParseException("then не найдено");
    }
    ...
    std::string condition = str.substr(kIfLength + 1, thenPos - 1);
    ...
    for (const auto& word : forbidden)
    {
        ...
        {
            throw ParseException("ошибка в конструкции " + conOperator + " then");
        }
    }
...
bool StringManipulation::CheckIfThenString(std::string& str)
{
    return CheckWithThenString(str, "if");
}

bool StringManipulation::CheckElseifThenString(std::string& str)
{
    return CheckWithThenString(str, "elseif");
}
